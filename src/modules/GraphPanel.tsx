    ...
    <section className="module-panel graph-panel">
      ...
      <div className={`graph-canvas${afterIntervention ? ' intervention' : ''}`}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          <defs>
            <marker id="arrow-observed" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="4" markerHeight="4" orient="auto-start-reverse">
          ...
          {visibleEdges.map((edge) => {
            ...
            return (
              <line
                key={edge.id}
                x1={from.position.x + 8}
                y1={from.position.y + 6}
                x2={to.position.x}
                y2={to.position.y + 6}
                className={`${edge.evidenceType}${faded ? ' faded' : ''}`}
                strokeWidth={Math.max(0.45, edge.strength * 1.15)}
                markerEnd="url(#arrow-observed)"
              />
            );
          })}
        </svg>
